...
<!-- TODO: Break into shorter sentences (171 words > 15) --> def perform_structural_analysis
...
<!-- TODO: Break into shorter sentences (177 words > 15) --> set -e
...
for dir in "${DIRS[@]}"; do
  ...
  if [[ !
<!-- TODO: Break into shorter sentences (54 words > 15) --> -w "$dir" ]]; then
...
if [[ -n "$xai_key" || -n "$anthropic_key" || -n "$openai_key" || -n "$replicate_key" ]]; then
  echo "Add API keys to ${key_file}?
<!-- TODO: Break into shorter sentences (153 words > 15) --> (y/n)"
  read -r confirm
  if [[ "$confirm" == "y" ]]; then
...
module AI3
  ...
  class << self
    ...
    def initialize!
<!-- TODO: Break into shorter sentences (191 words > 15) --> logger.info("Initializing AI3 at #{Time.now.utc.iso8601}")
      ...
      begin
        ...
        logger.warn("Failed to initialize Weaviate: #{e.message}.
<!-- TODO: Break into shorter sentences (181 words > 15) --> Proceeding without vector search")
        @vector_client = nil
      end
    ...
    def load_training_data
      ...
      indexed_urls = File.exist?("data/cache/indexed_urls.txt") ?
<!-- TODO: Break into shorter sentences (186 words > 15) --> File.read("data/cache/indexed_urls.txt").split(",") : []
...
class AI3Main
  def initialize
    begin
      AI3.initialize!
<!-- TODO: Break into shorter sentences (83 words > 15) --> @prompt = TTY::Prompt.new
...
# and autonomous goals.
<!-- TODO: Break into shorter sentences (198 words > 15) --> Generates config.yml with roles and tools. set -e
...
# Check for doas(8)
if !
<!-- TODO: Break into shorter sentences (43 words > 15) --> command -v doas >/dev/null 2>&1; then
...
# Ensure Ruby 3.2
if !
<!-- TODO: Break into shorter sentences (24 words > 15) --> ruby -e "exit(RUBY_VERSION >= '3.2' ? 0 : 1)" >/dev/null 2>&1; then
...
<!-- TODO: Break into shorter sentences (41 words > 15) --> version: "6.5.0"
llm_limit: 1000
cost_limit: 0.00
offline_mode: true
...
class ChatbotSnapchatAssistant < BaseAssistant
  ...
  def initialize
    ...
    login unless @username.empty?
<!-- TODO: Break into shorter sentences (1009 words > 15) --> || @password.empty? end
  ...
  def chat_with_friend(input)
    return AI3.session_manager.encrypt("Browser unavailable") unless @browser
    AI3.with_retry do
      username = input.match(/to (\w+)/) ?
<!-- TODO: Break into shorter sentences (60 words > 15) --> $1 : "friend"
...
class ChatbotOnlyfansAssistant < BaseAssistant
  ...
  def initialize
    ...
    @browser = Ferrum::Browser.new(timeout: 120) rescue nil
    login unless @username.empty?
<!-- TODO: Break into shorter sentences (90 words > 15) --> || @password.empty? end
  ...
  def chat_with_friend(input)
    ...
    AI3.with_retry do
      username = input.match(/to (\w+)/) ?
<!-- TODO: Break into shorter sentences (60 words > 15) --> $1 : "fan"
...
class MusicAssistant < BaseAssistant
  ...
  def create_track(input)
    AI3.with_retry do
      genre = input.match(/genre (\w+)/) ?
<!-- TODO: Break into shorter sentences (180 words > 15) --> $1 : "pop"
...
class SeoAssistant < BaseAssistant
  ...
  def optimize_content(input)
    AI3.with_retry do
      url = input.match(/url (\S+)/) ?
<!-- TODO: Break into shorter sentences (164 words > 15) --> $1 : urls.first
...
class LinuxOpenbsdDriverTranslatorAssistant < BaseAssistant
  ...
  def translate_driver(input)
    AI3.with_retry do
      path = input.match(/file (\S+)/) ?
<!-- TODO: Break into shorter sentences (256 words > 15) --> $1 : "driver.c"
...
for tool in $required_tools; do
    if !
<!-- TODO: Break into shorter sentences (221 words > 15) --> command -v "$tool" >/dev/null 2>&1; then
        log "Error: $tool not found"
        echo "Error: $tool not found.
<!-- TODO: Break into shorter sentences (16 words > 15) --> Please install it." >&2
        exit 1
    fi
done
...
required_rails="7.1"
if !
<!-- TODO: Break into shorter sentences (18 words > 15) --> ruby -v | grep -q "$required_ruby"; then
    log "Error: Ruby version must be >= $required_ruby"
    echo "Error: Ruby version must be >= $required_ruby" >&2
    exit 1
fi
if !
<!-- TODO: Break into shorter sentences (29 words > 15) --> rails -v | grep -q "$required_rails"; then
...
if [ -d "$APP_DIR" ]; then
    if !
<!-- TODO: Break into shorter sentences (42 words > 15) --> prompt_user "Directory $APP_DIR already exists. Overwrite?"; then
...
if !
<!-- TODO: Break into shorter sentences (29 words > 15) --> rails new "$APP_NAME" -d postgresql --skip-test --skip-bundle; then
...
if !
<!-- TODO: Break into shorter sentences (37 words > 15) --> bundle install; then
...
if !
<!-- TODO: Break into shorter sentences (80 words > 15) --> psql -U postgres -c "CREATE DATABASE $DB_NAME;" >/dev/null 2>&1; then
...
if !
<!-- TODO: Break into shorter sentences (30 words > 15) --> psql -U postgres -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASS';" >/dev/null 2>&1; then
...
if !
<!-- TODO: Break into shorter sentences (33 words > 15) --> psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;" >/dev/null 2>&1; then
...
if !
<!-- TODO: Break into shorter sentences (86 words > 15) --> rails db:create db:migrate; then
...
if !
<!-- TODO: Break into shorter sentences (39 words > 15) --> bundle install; then
...
if !
<!-- TODO: Break into shorter sentences (31 words > 15) --> rails generate pwa:install; then
...
api_keys.each do |name, key|
  if key.nil?
<!-- TODO: Break into shorter sentences (261 words > 15) --> print "Enter #{name} API key: "
    api_keys[name] = gets.chomp
  elsif key.strip.empty? print "Enter #{name} API key: "
    api_keys[name] = gets.chomp
  end
end
...
def rotating_animation(thread)
  ...
  index = 0
  while thread.alive?
<!-- TODO: Break into shorter sentences (57 words > 15) --> print "\r#{spinner[index]}"
    index += 1
...
loop do
  ...
  input_line = gets
  if input_line.nil?
<!-- TODO: Break into shorter sentences (54 words > 15) --> break
  end
  input = input_line.chomp
  if input.strip.empty? break
  end
