...
class CognitiveLoadReducer
  ...
  def measure_cognitive_load
    ...
    if cognitive_config
      cognitive_config['max_concepts_per_section'] || 7
      cognitive_config['max_nesting_depth'] || 3
  ...
  def create_cognitive_monitor
    ...
    File.chmod(0o755, 'cognitive_monitor.rb')
  ...
  def update_documentation_references
    ...
    readme_files.each do |readme|
      ...
      updated_content = content.gsub(%r{copilot/fix-[a-f0-9-]+}, 'main')
  ...
  def check_master_compliance
    ...
    compliance_items = [
      { check: 'cognitive_framework_enabled',
        result: @master_config.dig('core', 'cognitive_framework', 'compliance_level') == 'master' },
      { check: 'circuit_breakers_enabled', result: @master_config.dig('core', 'circuit_breakers', 'enabled') == true },
      { check: 'cognitive_load_budgeting',
        result: @master_config.dig('core', 'cognitive_load_budgeting', 'enabled') == true },
      { check: 'pitfall_prevention_active',
        result: @master_config.dig('core', 'pitfall_prevention', 'enabled') == true }
    ]
