# § Output Sh 2025-07-03

## `backup.sh`
```
#!/usr/bin/env zsh
# Archives folders to dated .tgz files, skips unchanged ones.
<!-- TODO: Break into shorter sentences (20 words > 15) --> # Usage: ./backup.sh [directory]
...
<!-- TODO: Break into shorter sentences (36 words > 15) --> typeset -A old_checksums
...
<!-- TODO: Break into shorter sentences (39 words > 15) --> checksum=$(find "$folder" -type f -exec md5 -q {} + | sort | md5 -q)
  ...
  if [[ -z "${old_checksums[$folder]}" || "${old_checksums[$folder]}" != "$checksum" ]]; then
    ...
    if [[ $?
<!-- TODO: Break into shorter sentences (40 words > 15) --> -ne 0 ]]; then
...
<!-- TODO: Break into shorter sentences (32 words > 15) --> for folder in ${(k)new_checksums}; do
...
<!-- TODO: Break into shorter sentences (30 words > 15) --> # Usage: ./clean.sh [target_folder]
...
<!-- TODO: Break into shorter sentences (20 words > 15) --> tr -d '\r' < "$file" | awk '{sub(/[ \t]+$/, "");} NF{print; if(p)print ""} {p=NF}' > "$tmp" 2>/dev/null
    if [[ $?
<!-- TODO: Break into shorter sentences (20 words > 15) --> -eq 0 ]]; then
      mv "$tmp" "$file"
      echo "Cleaned: $file"
    else
      rm "$tmp"
...
<!-- TODO: Break into shorter sentences (35 words > 15) --> # Usage: ./free_up_space.sh [directory]
...
<!-- TODO: Break into shorter sentences (28 words > 15) --> find "$search_dir" -type f -exec du -k {} + 2>/dev/null | sort -nr | head -n 10 | while read -r size path; do
...
echo "\nDelete?
<!-- TODO: Break into shorter sentences (64 words > 15) --> (y/N)"
read -r response
if [[ ! "$response" =~ ^[Yy]$ ]]; then
  echo "Done."
  exit 0
...
<!-- TODO: Break into shorter sentences (112 words > 15) --> # Usage: ./hack.sh [pattern]
...
if (( ${#files_to_open} > 0 )); then
  ...
  echo "Open in Vim?
<!-- TODO: Break into shorter sentences (82 words > 15) --> (Y/n)"
  read -r response
...
<!-- TODO: Break into shorter sentences (41 words > 15) --> # Usage: ./install.sh
...
#!/usr/bin/env zsh
# Archives folders to dated .tgz files, skips unchanged ones.
<!-- TODO: Break into shorter sentences (33 words > 15) --> # Usage: ./backup.sh [directory]
...
#!/usr/bin/env zsh
# Removes carriage returns, trailing whitespaces, and extra blank lines from text files.
<!-- TODO: Break into shorter sentences (37 words > 15) --> # Usage: ./clean.sh [target_folder]
...
#!/usr/bin/env zsh
# Finds text files matching a pattern and opens them in Vim.
<!-- TODO: Break into shorter sentences (44 words > 15) --> # Usage: ./hack.sh [pattern]
...
<!-- TODO: Break into shorter sentences (45 words > 15) --> # Usage: ./lint.sh
...
find .
<!-- TODO: Break into shorter sentences (21 words > 15) --> -type f \( -name "*.rb" -o -name "*.erb" \) \
...
<!-- TODO: Break into shorter sentences (30 words > 15) --> # Usage: ./perms.sh <owner> <group> <file_perms> <folder_perms>
...
if [[ !
<!-- TODO: Break into shorter sentences (35 words > 15) --> "$file_perms" =~ ^[0-7]{3}$ ]]; then
  echo "Error: File perms must be 3 digits (e.g., 644)"
  exit 1
fi
if [[ !
<!-- TODO: Break into shorter sentences (21 words > 15) --> "$folder_perms" =~ ^[0-7]{3}$ ]]; then
...
echo "Apply?
<!-- TODO: Break into shorter sentences (34 words > 15) --> (y/N)"
read -r confirm
if [[ "$confirm" =~ ^[Yy]$ ]]; then
...
<!-- TODO: Break into shorter sentences (38 words > 15) --> # Usage: ./replace.sh [-b] <old> <new> [folder]
...
if [[ !
<!-- TODO: Break into shorter sentences (59 words > 15) --> -d "$folder" ]]; then
...
for file in "$folder"/**/*(.N); do
  if "$is_filename"; then
    ...
    if [[ "$file" != "$new_file" && !
<!-- TODO: Break into shorter sentences (33 words > 15) --> -e "$new_file" ]]; then
    ...
    if [[ $is_text -eq 0 ]]; then
      if grep -q "$old_str" "$file" 2>/dev/null; then
        ...
        if [[ $?
<!-- TODO: Break into shorter sentences (61 words > 15) --> -eq 0 ]]; then
          mv "$file.tmp" "$file"
          echo "Updated: $file"
        else
          echo "Failed: $file"
...
<!-- TODO: Break into shorter sentences (46 words > 15) --> # Usage: ./showp.sh
...
if [[ $?
<!-- TODO: Break into shorter sentences (61 words > 15) --> -ne 0 ]]; then
...
<!-- TODO: Break into shorter sentences (37 words > 15) --> # Usage: ./svgomg.sh [folder]
...
for svg in "$dir"/**/*.svg(.N); do
  ...
  if [[ $?
<!-- TODO: Break into shorter sentences (26 words > 15) --> -eq 0 ]]; then
    echo "Processed: $svg"
  else
...
<!-- TODO: Break into shorter sentences (36 words > 15) --> # Usage: ./tree.sh [folder] [-a]
...
if [[ !
<!-- TODO: Break into shorter sentences (31 words > 15) --> -d "$folder" ]]; then
  echo "Error: '$folder' is not a directory"
  exit 1
fi
...
print_tree() {
  ...
  for entry in "$dir"/*(N); do
    if [[ !
<!-- TODO: Break into shorter sentences (27 words > 15) --> -e "$entry" ]]; then
      continue
    fi
...
#!/usr/bin/env zsh
# Finds and deletes large files to free up space.
<!-- TODO: Break into shorter sentences (51 words > 15) --> # Usage: ./free_up_space.sh [directory]
...
target_dir="$HOME/bin"
if [[ !
<!-- TODO: Break into shorter sentences (118 words > 15) --> -d "$target_dir" ]]; then
...
for script in *.sh; do
  ...
  if [[ $?
<!-- TODO: Break into shorter sentences (45 words > 15) --> -eq 0 ]]; then
    echo "Deployed: $dest"
  else
...
<!-- TODO: Break into shorter sentences (44 words > 15) --> path_line="export PATH=\"\$HOME/bin:\$PATH\""
...
<!-- TODO: Break into shorter sentences (44 words > 15) --> Check $log_file for errors."

# All scripts installed. echo "All scripts installed."
...
#!/usr/bin/env zsh
# Checks and fixes Ruby code files for errors.
<!-- TODO: Break into shorter sentences (19 words > 15) --> # Usage: ./lint.sh
...
#!/usr/bin/env zsh
# Changes file ownership and permissions.
<!-- TODO: Break into shorter sentences (23 words > 15) --> # Usage: ./perms.sh <owner> <group> <file_perms> <folder_perms>
...
#!/usr/bin/env zsh
# Swaps out words in files or renames them.
<!-- TODO: Break into shorter sentences (31 words > 15) --> # Usage: ./replace.sh [-b] <old> <new> [folder]
...
#!/usr/bin/env zsh
# Creates a Markdown list of text files and their contents.
<!-- TODO: Break into shorter sentences (39 words > 15) --> # Usage: ./showp.sh
...
#!/usr/bin/env zsh
# Shrinks SVG files to save space.
<!-- TODO: Break into shorter sentences (30 words > 15) --> # Usage: ./svgomg.sh [folder]
...
#!/usr/bin/env zsh
# Displays a tree of directories and files.
<!-- TODO: Break into shorter sentences (29 words > 15) --> # Usage: ./tree.sh [folder] [-a]
